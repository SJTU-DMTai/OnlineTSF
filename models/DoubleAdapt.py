...
import higher
import util.higher_optim  # IMPORTANT, DO NOT DELETE
from util.tools import has_rnn, override_state
...
class IncrementalManager:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.opt = torch.optim.Adam(self.model.parameters(), lr=lr_model, weight_decay=kwargs.get('weight_decay'))
        self.meta_opt = self._init_meta_optimizer(**kwargs)
    ...
    def state_dict(self, destination: typing.OrderedDict[str, torch.Tensor]=None, prefix='', keep_vars=False):
        ...
        destination['meta_opt'] = self.meta_opt.state_dict()
        return destination
    ...
    def load_state_dict(self, state_dict: typing.OrderedDict[str, torch.Tensor],):
        ...
        """
        self.model.load_state_dict(state_dict['framework'])
        self.opt.load_state_dict(state_dict['framework_opt'])
        self.meta_opt.load_state_dict(state_dict['opt'])
    ...
    def _run_task(self, meta_input: Dict[str, Union[pd.Index, torch.Tensor]], phase: str):
        ...
        self.opt.zero_grad()
        y_hat = self.model(meta_input["X_train"].to(self.model.device), None)
        loss = self.model.criterion(y_hat, meta_input["y_train"].to(self.model.device))
        loss.backward()
        self.opt.step()
        self.opt.zero_grad()
        with torch.no_grad():
            pred = self.model(meta_input["X_test"].to(self.model.device), None)
...
class DoubleAdaptManager(IncrementalManager):
    ...
    def _init_meta_optimizer(self, lr_da=0.01, lr_x=None, lr_y=None, **kwargs):
        """ NOTE: the optimizer of the model adapter is self.opt """
        if lr_x is None or lr_y is None:
            return optim.Adam(self.model.meta_params, lr=lr_da)
        else:
            return optim.Adam([{'params': self.model.teacher_x.parameters(), 'lr': lr_x},
                               {'params': self.model.teacher_y.parameters(), 'lr': lr_y},])
    ...
    def override_online_lr_(self):
        if self.online_lr is not None:
            ...
            if 'lr_ma' in self.online_lr:
                self.opt.param_groups[0]['lr'] = self.online_lr['lr_ma']
            if 'lr_da' in self.online_lr:
                self.meta_opt.param_groups[0]['lr'] = self.online_lr['lr_da']
            else:
                if 'lr_x' in self.online_lr:
                    self.meta_opt.param_groups[0]['lr'] = self.online_lr['lr_x']
                if 'lr_y' in self.online_lr:
                    self.meta_opt.param_groups[1]['lr'] = self.online_lr['lr_y']
    ...
    def _run_task(self, meta_input: Dict[str, Union[pd.Index, torch.Tensor]], phase: str):
        ...
        X = meta_input["X_train"].to(self.model.device)
        with higher.innerloop_ctx(
            ...
            with torch.backends.cudnn.flags(enabled=self.first_order or not self.has_rnn):
                y_hat, _ = self.model(X, model=fmodel, transform=self.adapt_x)
        y = meta_input["y_train"].to(self.model.device)
        if self.adapt_y:
            raw_y = y
            y = self.model.teacher_y(X, raw_y, inverse=False)
        loss2 = self.model.criterion(y_hat, y)
        diffopt.step(loss2)

        """ Online inference """
        if phase != "train" and "X_extra" in meta_input and meta_input["X_extra"].shape[0] > 0:
            X_test = torch.cat([meta_input["X_extra"].to(self.model.device), meta_input["X_test"].to(self.model.device), ], 0, )
            y_test = torch.cat([meta_input["y_extra"].to(self.model.device), meta_input["y_test"].to(self.model.device), ], 0, )
        else:
            X_test = meta_input["X_test"].to(self.model.device)
            y_test = meta_input["y_test"].to(self.model.device)
        pred, X_test_adapted = self.model(X_test, model=fmodel, transform=self.adapt_x)
        if self.adapt_y:
            pred = self.model.teacher_y(X_test, pred, inverse=True)
        mask_y = None
        if phase != "train":
        ...
        if len(y_test) == 0:
            ...
            self.model.model.load_state_dict(fmodel.state_dict())
            self.opt.state = override_state(self.opt.param_groups, diffopt)
            return output

        loss = self.model.criterion(pred, y_test)
        if self.adapt_y:
            ...
            if self.first_order:
                ...
                with torch.no_grad():
                    pred2, _ = self.model(X_test_adapted, model=None, transform=False, )
                    pred2 = self.model.teacher_y(X_test, pred2, inverse=True).detach()
                    if mask_y is not None:
                        pred2 = pred2[mask_y[:meta_end]]
                    loss_old = self.model.criterion(pred2.view_as(y_test), y_test)
